...
export default function Home() {
  ...
  const onFileEncrypt = async () => {
    try {
      if (file && password) {
        setLoading(true);
        const encryptedFile = await decryptFile(file, password);
        if (!encryptedFile) {
          ...
          <div
            ...
            <div className="relative">
              <div className="sm:text-center">
                <h2 className="text-3xl font-extrabold text-white tracking-tight sm:text-4xl">Decrypt your file!</h2>
                <p className="mt-6 mx-auto max-w-2xl text-lg text-indigo-200">
                  Upload an encrypted file that you want to decrypt, then type the password to the input field which we
                  can use for the decryption. After these, you can click on the decrypt button to download the decrypted
                  file.
                </p>
              </div>
              ...
              <div className="mt-12 flex justify-center">
                <Link href="/encrypt">
                  <a className="text-base  text-white hover:text-gray-200">I want to encrypt my file</a>
                </Link>
              </div>
            </div>
          </div>
